...
// ------------------------------------------------------------------
// VALIBOT FIREWALL SCHEMA
// ------------------------------------------------------------------
const FeedItemSchema = v.object({
  id: v.optional(v.string()), 
  food_item: v.optional(v.string()), 
  feed_method: v.optional(v.string()),
  quantity: v.optional(v.pipe(v.number(), v.minValue(0, 'Cannot be negative'))),
  unit: v.picklist(['grams', 'whole_item']),
  calci_dust_added: v.optional(v.boolean(), false),
});

const FeedGroupSchema = v.pipe(
  v.object({
    recorded_by: v.pipe(v.string(), v.uuid("ZLA COMPLIANCE: An active staff member must be selected.")),
    recorded_at: v.pipe(v.string(), v.minLength(1, 'Date and Time required')),
    outcome: v.picklist(['EATEN', 'REFUSED', 'FASTING', 'NOT_CAST', 'REGURGITATED']),
    items: v.pipe(v.array(FeedItemSchema), v.minLength(1, 'At least one item component required')),
  }),
  v.check((data) => {
    if (data.outcome === 'EATEN') {
      for (const item of data.items) {
        if (!item.food_item || item.food_item.trim() === '') return false;
        if (item.quantity === undefined || item.quantity <= 0) return false;
      }
    }
    return true;
  }, 'Food item and positive quantity (> 0) required when outcome is Eaten')
);

type FeedFormValues = v.InferOutput<typeof FeedGroupSchema>;
...
export function FeedModal({ isOpen, onClose, animalId, animal: passedAnimal, initialData, scheduledFeed, selectedDate }: FeedModalProps) {
  ...
  const { data: opLists = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        const fallback = await supabase.from('operational_list').select('name, category, animal_category');
    ...
    mutationFn: async (values: FeedFormValues) => {
      const payloads = values.items.map(item => {
        ...
        return {
          ...
          modified_by: profile?.id || values.recorded_by,
  ...
  const form = useForm<FeedFormValues>({
    ...
    validators: {
      onSubmit: ({ value }) => {
        const result = v.safeParse(FeedGroupSchema, value);
        if (!result.success) {
