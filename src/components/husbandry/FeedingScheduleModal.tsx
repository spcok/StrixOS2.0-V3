...
const getAnimalsOptions = () => queryOptions({
  ...
  queryFn: async () => {
    ...
    if (error) {
      const fallback = await supabase.from('animals').select('*').eq('is_deleted', false);
      if (fallback.error) throw error;
      return (fallback.data || []) as Animal[];
    }
...
const getFoodOptions = () => queryOptions({
  ...
  queryFn: async () => {
    const { data, error } = await supabase
      ...
      .order('name');
    if (error) {
      const fallback = await supabase.from('operational_list').select('*').order('name');
      if (fallback.error) return [] as OperationalList[];
      return fallback.data as OperationalList[];
    }
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        if (!userId) throw new Error('You must be logged in to schedule diets.');
        if (!value.animal_id) throw new Error('Please select an animal from the dropdown.');
        if (!value.feed_not_required && !value.food_type) {
          throw new Error('Please select a food item from the dropdown.');
        }
        ...
        await feedingService.bulkCreateSchedules(newSchedules as any, userId);
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-xl border border-slate-100 flex flex-col my-8">
        ...
        <form onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); form.handleSubmit(); }} className="p-4 md:p-6 space-y-4 max-h-[75vh] overflow-y-auto custom-scrollbar">
          ...
          <form.Subscribe selector={(state) => [state.values.feed_not_required, state.values.animal_id]} children={([notRequired, selectedAnimalId]) => {
            if (notRequired) return null;

            const matchedAnimal = animals.find(a => a.id === selectedAnimalId);
            const animalCat = matchedAnimal?.category?.toUpperCase().trim() || (activeCategory !== 'ALL' ? activeCategory.toUpperCase() : '');

            const filteredFoodOptions = foodOptions.filter((f: any) => {
              const cat = (f.category || '').toLowerCase();
              if (cat !== 'food_type' && cat !== 'food') return false;
              if (!f.animal_category) return true;
              return animalCat ? String(f.animal_category).toUpperCase().includes(animalCat) : true;
            });
                  ...
                  <form.Field name="food_type" children={(field) => (
                    <div>
                      ...
                      <select value={field.state.value} onBlur={field.handleBlur} onChange={e => field.handleChange(e.target.value)} className={inputClass} disabled={loadingFood} required>
                        <option value="">{loadingFood ? 'Loading...' : '-- Select Food Item --'}</option>
                        {filteredFoodOptions.map(f => <option key={f.id || f.name} value={f.name}>{f.name}</option>)}
                      </select>
                    </div>
                  )}/>
          ...
          <div className="pt-2 flex gap-2.5">
            ...
            <form.Subscribe selector={(state) => [state.canSubmit, state.isSubmitting, state.values]} children={([canSubmit, isSubmitting, values]) => {
              const disableSubmit = !canSubmit || (isSubmitting as boolean) || !values.animal_id || (!values.feed_not_required && !values.food_type) || (values.schedule_mode === 'specific_days' && values.selected_days.length === 0);
              return (
