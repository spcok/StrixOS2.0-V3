...
type TabType = 'profile' | 'husbandry' | 'events' | 'training';

const formatWeight = (val: number | null | undefined, unit?: string | null) => {
  if (val === null || val === undefined) return '--';
  return `${val}${unit || 'g'}`;
};

export function AnimalProfile({ animalId, id, animal: passedAnimal, onBack, onClose }: AnimalProfileProps) {
  ...
  const { data: animal, isLoading } = useQuery<Animal | null>({
    ...
    initialData: passedAnimal || undefined,
    staleTime: 1000 * 60 * 2,
  });
  ...
  const archiveMutation = useMutation({
    mutationFn: async () => {
      ...
      // 1. Archive the animal record locally
      ...
      if (isDeath) {
        const { error: deathError } = await supabase
          ...
          .insert({
            ...
            necropsy_notes: necropsyNotes || null,
    ...
    onError: (err: unknown) => {
      const msg = err instanceof Error ? err.message : 'Failed to process disposition.';
      toast.error(msg);
  ...
  if (isLoading) {
    return (
      <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-md flex items-center justify-center">
      ...
      <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl p-8 max-w-md w-full text-center shadow-2xl border border-slate-200">
          ...
          <button 
            type="button" 
            onClick={handleClose} 
            className="px-6 py-2.5 bg-slate-100 text-slate-700 rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-slate-200 cursor-pointer"
          >
            Close
          </button>
        </div>
  ...
  return (
    <div className="fixed inset-0 z-40 bg-slate-950/70 backdrop-blur-md overflow-y-auto pt-6 pb-12 px-4 custom-scrollbar font-sans">
      <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in zoom-in-95 duration-200 relative">
        ...
        {handleClose && (
          <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-slate-300">
            <button 
              type="button" 
              onClick={handleClose} 
              className="flex items-center gap-2 hover:text-white transition-colors bg-slate-800/80 hover:bg-slate-800 px-3 py-1.5 rounded-xl border border-slate-700 cursor-pointer shadow-sm"
            >
        ...
        {/* HEADER CARD */}
        <div className="bg-white rounded-3xl p-5 flex flex-col md:flex-row gap-8 shadow-2xl border border-slate-200/80">
          <div className="w-full md:w-[280px] h-[280px] shrink-0 rounded-2xl overflow-hidden bg-slate-100 shadow-inner border border-slate-200 relative">
            <img 
          ...
          <div className="flex-1 flex flex-col pt-2 relative">
            <div className="absolute top-0 right-0 flex items-center gap-2">
              <button 
                ...
                className="p-2.5 text-blue-600 border border-slate-200 rounded-xl hover:bg-blue-50 hover:border-blue-200 transition-all shadow-sm bg-white cursor-pointer" 
                title="Generate Signage"
              >
              ...
              <button 
                ...
                className="p-2.5 text-emerald-600 border border-slate-200 rounded-xl hover:bg-emerald-50 hover:border-emerald-200 transition-all shadow-sm bg-white cursor-pointer" 
                title="Edit Profile"
              >
                <Edit2 size={16} />
              </button>
              ...
              <button 
                ...
                className="p-2.5 border rounded-xl transition-all shadow-sm text-rose-600 border-slate-200 bg-white hover:bg-rose-50 hover:border-rose-200 cursor-pointer"
              ...
              {handleClose && (
                <button 
                  type="button"
                  onClick={handleClose} 
                  className="p-2.5 text-slate-400 border border-slate-200 rounded-xl hover:bg-slate-100 hover:text-slate-700 transition-all shadow-sm bg-white ml-2 cursor-pointer"
                >
                  <X size={16} />
                </button>
              )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-y-6 gap-x-4">
              <div>
                ...
                <span className="text-sm font-bold text-slate-900">{animal.species || 'Unknown'}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="text-sm font-bold text-slate-900 uppercase">{animal.gender || 'UNKNOWN'}</span>
              </div>
              <div>
                ...
                <span className="text-sm font-bold text-slate-900">
                  {animal.is_dob_unknown 
                    ? 'Unknown' 
                    : animal.date_of_birth && !Number.isNaN(new Date(animal.date_of_birth).getTime()) 
                      ? new Date(animal.date_of_birth).toLocaleDateString('en-GB') 
                      : 'Unrecorded'}
        ...
        {/* TABS & BODY */}
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col min-h-[500px] border border-slate-200/80">
          <div className="flex border-b border-slate-100 px-2 pt-2 bg-slate-50 shrink-0 overflow-x-auto custom-scrollbar">
            {[
              { id: 'profile' as const, label: 'Profile Matrix', icon: FileText },
              { id: 'husbandry' as const, label: 'Husbandry Logs', icon: ClipboardList },
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-4 text-[11px] font-black uppercase tracking-widest transition-all border-b-2 whitespace-nowrap cursor-pointer ${
                    isActive 
                      ? 'border-emerald-500 text-emerald-600 bg-white rounded-t-2xl shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.02)]' 
          ...
          <div className="p-6 bg-white flex-1 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="bg-rose-50/80 border border-rose-200 rounded-2xl p-5 shadow-sm">
                      ...
                      <div className="text-sm font-bold text-rose-800 space-y-2">
                        {Array.isArray(animal.critical_husbandry_notes) 
                          ? animal.critical_husbandry_notes.map((n: string, i: number) => (
                  ...
                  {animal.category === 'EXOTIC' && (
                    <div className="bg-orange-50/30 border border-orange-200 rounded-2xl p-5 shadow-sm">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div>
                          <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Humidity Target</span>
                          <span className="text-sm font-black text-slate-800">
                            {animal.target_humidity_min_percent 
                              ? `${animal.target_humidity_min_percent}%${animal.target_humidity_max_percent ? ` - ${animal.target_humidity_max_percent}%` : ''}` 
                              : '--'}
                          </span>
                        </div>
                        <div>
                          <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-1">
                            <Droplets size={10} /> Misting
                          </span>
                          <span className="text-sm font-black text-slate-800">
                            {animal.misting_not_required ? 'Not Required' : (animal.misting_frequency || (animal.ambient_temp_only ? 'Ambient Only' : 'Active Protocol'))}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                        ...
                        <IUCNBadge status={animal.red_list_status || animal.iucn_status} />
                      </div>
                      {animal.is_venomous && (
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <span className="text-xs font-mono text-slate-800 truncate block" title={animal.sire_id || 'Unknown'}>
                            {animal.sire_id ? `${animal.sire_id.substring(0, 8)}...` : 'Unknown'}
                          </span>
                        </div>
                        <div>
                          <span className="block text-[9px] font-black uppercase tracking-widest text-slate-400">Dam ID</span>
                          <span className="text-xs font-mono text-slate-800 truncate block" title={animal.dam_id || 'Unknown'}>
                            {animal.dam_id ? `${animal.dam_id.substring(0, 8)}...` : 'Unknown'}
                          </span>
                        </div>
                      </div>
                      <div className="pt-2 border-t border-slate-100">
                        <span className="block text-[9px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1 mb-1">
                          <MapPin size={10} /> Origin / Acquisition
                        </span>
                        <span className="text-sm font-bold text-slate-800 block">{animal.origin || 'Unknown Source'}</span>
                        <span className="text-xs text-slate-500 font-medium">
                          {animal.acquisition_date ? new Date(animal.acquisition_date).toLocaleDateString('en-GB') : 'No date'} ({animal.acquisition_type || 'Unknown Type'})
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            {activeTab === 'events' && (
              <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-3">
                ...
                <p className="text-[10px] font-bold mt-1 text-center">Displays, public interactions, and scheduled event logs.</p>
              </div>
            )}
            ...
            {activeTab === 'training' && (
              <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-3">
                ...
                <p className="text-[10px] font-bold mt-1 text-center">Operant conditioning, flight weight schedules, and recall scores.</p>
              </div>
            )}
      ...
      {isArchiveConfirmOpen && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md overflow-y-auto">
          <div className="bg-white rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200 my-auto border border-slate-200">
            <div className="flex flex-col">
              
              <div className="flex items-center gap-3 mb-6">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${isDeath ? 'bg-slate-100 text-slate-700' : 'bg-rose-50 text-rose-500'}`}>
              ...
              <div className="space-y-4 mb-8">
                <div>
                  <label htmlFor="disposition-reason-select" className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">
                  ...
                  <select 
                    ...
                    className={`w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-bold text-slate-900 focus:outline-none focus:ring-2 shadow-sm transition-colors cursor-pointer ${isDeath ? 'focus:border-slate-500 focus:ring-slate-500/20' : 'focus:border-rose-500 focus:ring-rose-500/20'}`}
                  >
                ...
                {isDeath ? (
                  <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div>
                      <label htmlFor="disposition-cause-of-death" className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 flex items-center gap-2">
                        Cause of Death <span className="text-[9px] text-slate-400 font-bold">(If Known)</span>
                      </label>
                      <input 
                        id="disposition-cause-of-death"
                        type="text" 
                    ...
                    <div>
                      <label htmlFor="disposition-necropsy-notes" className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">
                        Necropsy / Post-Mortem Notes
                      </label>
                      <textarea 
                        id="disposition-necropsy-notes"
                        rows={3}
                  ...
                  <div className={`animate-in fade-in slide-in-from-top-2 duration-200 ${!requiresDestination ? 'opacity-50' : ''}`}>
                    <label htmlFor="disposition-destination-input" className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">
                      Destination / Receiving Entity
                    </label>
                    <input 
                      id="disposition-destination-input"
                      type="text" 
                ...
                <div>
                  <label htmlFor="disposition-datetime-input" className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">
                    Date &amp; Time of Disposition
                  </label>
                  <input 
                    id="disposition-datetime-input"
                    type="datetime-local" 
                    value={archivedAt}
              ...
              <div className="flex items-center gap-3 w-full border-t border-slate-100 pt-5 mt-auto">
                <button 
                  ...
                  className="flex-1 px-4 py-3 bg-slate-100 text-slate-600 font-bold text-xs uppercase tracking-widest rounded-xl hover:bg-slate-200 transition-colors cursor-pointer"
                >
                  Cancel
                </button>
                <button 
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-colors shadow-sm disabled:opacity-50 cursor-pointer ${isDeath ? 'bg-slate-900 hover:bg-slate-800' : 'bg-rose-600 hover:bg-rose-500'}`}
                >
