...
export const IUCNBadge: FC<IUCNBadgeProps> = ({ status }) => {
  const normalized = status ? status.toUpperCase().trim() : 'NE';
  const config = STATUS_MAP[normalized] || STATUS_MAP.NE;

  return (
    <span
      className={`inline-flex items-center px-2.5 py-1 text-[9px] font-black uppercase tracking-widest border rounded-md shadow-xs select-none ${config.bg} ${config.color} ${config.border}`}
    >
      {config.label}
    </span>
  );
