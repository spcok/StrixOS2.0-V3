  ...
  loader: async ({ context: { queryClient } }: any) => {
    if (queryClient) {
      try {
        ...
        console.warn('[Maintenance Route Loader]: Offline hydration fallback active.', err);
      }
    }
  },
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 lg:flex lg:gap-2 w-full shrink-0 gap-1.5 overflow-x-auto pb-1 lg:pb-0">
        {tabs.map(tab => (
          <button
            ...
            className={`px-2 lg:px-4 py-1.5 lg:py-2 rounded-xl text-[9px] lg:text-xs font-black uppercase tracking-widest whitespace-nowrap lg:whitespace-normal transition-all shadow-sm flex items-center justify-center gap-1.5 cursor-pointer ${
              statusFilter === tab.id 
...
function MaintenanceModal({ onClose, staffMembers }: { onClose: () => void, staffMembers: any[] }) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: any) => {
      ...
      return await maintenanceService.saveTicket(payload, activeUserId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['maintenance_tickets'] });
