...
import type { SafetyDrill } from '../../types';
...
const safetyDrillsOptions = queryOptions({
  ...
  queryFn: async () => {
    return await safetyDrillService.getDrills();
  },
  staleTime: 1000 * 60 * 5,
  ...
  loader: async ({ context }: any) => {
    if (context?.queryClient) {
      try {
        ...
        console.warn('[SafetyDrills Route Loader]: Offline hydration fallback active.', err);
      }
    }
  },
...
export function SafetyDrillsPage() {
  ...
  useEffect(() => {
    const channel = supabase.channel('safety-drills-realtime')
  ...
  const createDrillMutation = useMutation({
    mutationFn: async (newDrill: any) => {
      ...
      return await safetyDrillService.saveDrill(newDrill, activeUserId);
    },
    onSuccess: () => {
    ...
    onError: (err: any) => {
      toast.error(`Failed to record drill: ${err.message || 'Database error'}`);
    }
  });
