...
export const Route = createFileRoute('/safety/incidents')({
  loader: async ({ context }: any) => {
    if (context?.queryClient) {
      try {
        await Promise.all([
          context.queryClient.ensureQueryData(incidentsOptions),
          context.queryClient.ensureQueryData(staffMembersOptions)
        ]);
      } catch (err) {
        console.warn('[Incidents Route Loader]: Offline hydration fallback active.', err);
      }
    }
  },
...
export function IncidentsPage() {
  ...
  const resolveMutation = useMutation({
    mutationFn: async ({ id, notes }: { id: string, notes: string }) => {
      await incidentService.resolveIncident(id, notes, user?.id);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['incidents'] });
  ...
  return (
    <div className="h-[calc(100vh-6rem)] flex flex-col space-y-5 lg:space-y-6 animate-in fade-in duration-500 w-full font-sans">
      ...
      <div className="flex justify-between items-start w-full mb-2 lg:mb-4 portrait:flex landscape:hidden lg:landscape:flex shrink-0">
        ...
        {hasPermission('safety:write') && (
          <button 
            ...
            className="flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-3 lg:px-4 py-2 lg:py-2.5 rounded-xl text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all shadow-sm active:scale-95 shrink-0 cursor-pointer"
          >
      ...
      <div className="grid grid-cols-3 lg:flex lg:gap-2 w-full shrink-0 gap-1.5 overflow-x-auto pb-1 lg:pb-0">
        {tabs.map(tab => (
          <button
            ...
            className={`px-1 lg:px-4 py-1.5 lg:py-2 rounded-xl text-[9px] lg:text-xs font-black uppercase tracking-widest whitespace-nowrap lg:whitespace-normal transition-all shadow-sm flex items-center justify-center gap-1.5 cursor-pointer ${
              statusFilter === tab.id 
                        ...
                        {isOpen ? (
                          hasPermission('safety:write') && (
                            <button 
                              ...
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-amber-200 bg-amber-50 text-[10px] font-black text-amber-700 uppercase tracking-widest hover:bg-amber-100 transition-colors shadow-sm active:scale-95 cursor-pointer"
                            >
...
function CompoundIncidentModal({ onClose, userId, staffMembers }: { onClose: () => void, userId?: string, staffMembers: StaffMember[] }) {
  ...
  const saveMutation = useMutation({
    mutationFn: async (payloads: { incident: any, firstAid?: any }) => {
      await incidentService.commitIncident(payloads.incident, payloads.firstAid, userId);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['incidents'] });
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 font-sans overflow-y-auto custom-scrollbar">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl flex flex-col shadow-2xl relative overflow-hidden my-auto animate-in zoom-in-95 duration-200">
        <div className="bg-slate-50 border-b border-slate-100 p-5 flex justify-between items-center shrink-0 rounded-t-2xl">
          ...
          <button type="button" onClick={onClose} className="p-1.5 text-slate-400 hover:text-slate-700 rounded-xl hover:bg-slate-100 transition-colors cursor-pointer">
            <X size={18} />
          </button>
        </div>
                  ...
                  <div className="flex bg-slate-50 border border-slate-200 rounded-xl p-1 shadow-sm flex-wrap gap-1">
                    {['ESCAPE', 'ANIMAL_BEHAVIOR', 'INFRASTRUCTURE', 'SLIP_TRIP_FALL', 'OTHER'].map((type) => (
                      <button 
                        ...
                        className={`flex-1 min-w-[100px] py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all cursor-pointer ${field.state.value === type ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200/60'}`}
                      >
                        {type.replace(/_/g, ' ')}
                      </button>
                    ))}
                    ...
                    <form.Field name="injury_description">
                      {(field) => (
                        <div>
                          ...
                          <textarea required value={field.state.value} onBlur={field.handleBlur} onChange={e => field.handleChange(e.target.value)} rows={2} className={`${inputClass} resize-none h-16`} placeholder="E.g., Laceration on left index finger..." />
                        </div>
                      )}
                    </form.Field>
                    ...
                    <form.Field name="treatment_provided">
                      {(field) => (
                        <div>
                          ...
                          <textarea required value={field.state.value} onBlur={field.handleBlur} onChange={e => field.handleChange(e.target.value)} rows={2} className={`${inputClass} resize-none h-16`} placeholder="E.g., Cleaned wound with sterile wipe, applied dressing..." />
                        </div>
                      )}
                    </form.Field>
        ...
        <div className="p-5 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 shrink-0 rounded-b-2xl">
          <button type="button" onClick={onClose} className="px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-slate-900 hover:bg-slate-200 rounded-xl transition-colors cursor-pointer">
            Cancel
          </button>
          <form.Subscribe selector={(state) => [state.canSubmit, state.isSubmitting, state.values.requires_first_aid]}>
            {([canSubmit, isSubmitting, hasFirstAid]) => (
              <button 
                ...
                className="px-6 py-2.5 bg-slate-900 hover:bg-slate-800 disabled:opacity-50 flex items-center gap-2 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-md active:scale-95 cursor-pointer"
              >
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 font-sans overflow-y-auto custom-scrollbar">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md flex flex-col shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-5 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 rounded-b-2xl">
          <button type="button" onClick={onClose} className="px-4 py-2 text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-slate-900 hover:bg-slate-200 rounded-xl transition-colors cursor-pointer">
            Cancel
          </button>
          <form.Subscribe selector={(state) => [state.canSubmit, state.isSubmitting]}>
            {([canSubmit, isSubmitting]) => (
              <button 
                ...
                className="px-5 py-2 bg-slate-900 hover:bg-slate-800 disabled:opacity-50 flex items-center gap-2 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-sm active:scale-95 cursor-pointer"
              >
